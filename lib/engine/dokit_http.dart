...
class DoKitHttpOverrides extends HttpOverrides {
  ...
  HttpClient createHttpClient(SecurityContext? context) {
    ...
    print(
        'createHttpClient kDebugMode $kDebugMode 代理 ip：${DoKitController.ip} port：${DoKitController.port}');

    ///设置代理
    if (kDebugMode &&
        DoKitController.ip.isNotEmpty &&
        DoKitController.port.isNotEmpty) {
      client.findProxy = (url) {
        return 'PROXY ${DoKitController.ip}:${DoKitController.port}';
      };
...
class DoKitHttpClient implements HttpClient {
  ...
  ///FlutterSDK3.7.0兼容，新增方法
  @override
  set connectionFactory(
      Future<ConnectionTask<Socket>> Function(
              Uri url, String? proxyHost, int? proxyPort)?
          f) {
    origin.connectionFactory = f as Future<ConnectionTask<Socket>> Function(
        Uri url, String? proxyHost, int? proxyPort);
  }
